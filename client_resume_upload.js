...
class MyReadable extends Readable {
    constructor(data, start, options) {
        super(options);
        this.data = data.slice(start, data.length);
        this.start = start;
    }
...
function run() {
    ...
    const objStream = new MyReadable(JSON.stringify(objectForUpload), 0, {
        objectMode: true,
        highWaterMark: 2,
    });
    ...
    //find hash of object it use for check valid data confirm the inside information is not modified and complete data.
    let obj_hash = hash(Buffer.from(JSON.stringify(objectForUpload)));
    console.log(obj_hash);

    let upload_request_data = JSON.stringify({
        hash: obj_hash,
    });

    let config_upload_request = {
        method: 'post',
        url: `${url_endpoint}/upload-request`,
        headers: {
    ...
    axios(config_upload_request)
        .then(function (response) {
            let response_msg = response.data;

            let new_fromdata = new FormData();



            let config_resume_position = {
                method: 'get',
                url: `${url_endpoint}/resume_position`,

            };
            if (response_msg === 'OK') {
                axios(config_resume_position)
                    .then(function (response) {
                        // console.log(JSON.stringify(response.data));
                        let response_msg = response.data;
                        if (response_msg['message'] === 'OK') {
                            let start = parseInt(response_msg['position'])

                            const objResumeStream = new MyReadable(JSON.stringify(objectForUpload), start, {
                                objectMode: true,
                                highWaterMark: 2,
                            });
                            ...
                            new_fromdata.append('file', objResumeStream, {
                                filename: 'obj.txt',
                            });

                            let config_upload = {
                                method: 'post',
                                url: `${url_endpoint}/upload`,
                                headers: {
                                    hash: obj_hash,
                                    ...new_fromdata.getHeaders(),
                                },
                                data: new_fromdata,
                            };
